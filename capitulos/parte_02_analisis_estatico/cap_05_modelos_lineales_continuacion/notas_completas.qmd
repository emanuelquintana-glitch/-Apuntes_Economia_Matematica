...
import sympy as sp
from sympy import Matrix, symbols, latex, simplify, solve, det, eye, inverse_ADJ
...
$$|C_{ij}| = (-1)^{i+j} |M_{ij}|$$
```{python}
# Cálculo de menores y cofactores
def calcular_menor(A, i, j):
    """Calcula el menor M_ij"""
    M = A.copy()
    M.row_del(i)
    M.col_del(j)
    return det(M)
...
def calcular_cofactor(A, i, j):
    """Calcula el cofactor C_ij"""
    return (-1)**(i+j) * calcular_menor(A, i, j)

# Ejemplo
A = Matrix([[9, 11, 4], [7, 6, 1], [3, 1, 4]])

print("Matriz A:")
display_matrix(A)

print("\nMenores y Cofactores del tercer renglón:")
for j in range(3):
    menor = calcular_menor(A, 2, j)
    cofactor = calcular_cofactor(A, 2, j)
    print(f"M_3{j+1} = {menor}, C_3{j+1} = {cofactor}")
<!-- ``` -->
...
$$|A| = \sum_{j=1}^{n} a_{ij} |C_{ij}|$$ (expansión por renglón $i$)

$$|A| = \sum_{i=1}^{n} a_{ij} |C_{ij}|$$ (expansión por columna $j$)
```{python}
# Expansión de Laplace por diferentes renglones/columnas
def expansion_laplace(A, fila=None, columna=None):
    """Calcula determinante mediante expansión de Laplace"""
    n = A.shape[0]
    ...
    if fila is not None:
        ...
        print(f"Expansión por renglón {fila + 1}:")
        return resultado
    elif columna is not None:
        resultado = 0
        for i in range(n):
            resultado += A[i, columna] * calcular_cofactor(A, i, columna)
        print(f"Expansión por columna {columna + 1}:")
        return resultado

A = Matrix([[1, 2, 0, 9], [2, 3, 4, 6], [1, 6, 0, -1], [0, -5, 0, 8]])

print("Matriz A (4×4):")
display_matrix(A)

print("\nDeterminante calculado por diferentes métodos:")
print(f"Sympy det(): {det(A)}")
print(f"Expansión por fila 1: {expansion_laplace(A, fila=0)}")
print(f"Expansión por columna 3: {expansion_laplace(A, columna=2)}")
```
